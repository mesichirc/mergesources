  ...
  PEPE_PROCESS_COMMANDS(iterator, command) {
    ...
    switch (command.type) {
      ...
      case PEPE_TEXT: {
        ...
        view.text = [[NSString alloc] initWithBytes:(void *)command.text.base length:command.text.length encoding:NSUTF8StringEncoding];
        break;
      }
      default: {}
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    Pepe_ContextDeclaration contextDeclaration = {0};

    u64 requredMemory = PEPE_REQUERED_MEMORY();
    contextDeclaration.memory = Pepe_SliceInit(mmap(nil, requredMemory, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANON, -1, 0), requredMemory);


    appContext.context = Pepe_ContextInit(contextDeclaration);
...
App_Layout(Pepe_Context *ctx)
{
  ...
  Pepe_OpenElement(ctx); // 1
    Pepe_OpenElement(ctx); // 2
      Pepe_OpenElement(ctx); // 3
        Pepe_OpenElement(ctx); // 4
        Pepe_CloseElement(ctx); // 4
      Pepe_CloseElement(ctx); // 3
                            
      Pepe_OpenElement(ctx); // 5
        element = Pepe_GetOpenedElement(ctx);
        element->options.frameType = PEPE_FRAME_TYPE_GROW;
      Pepe_CloseElement(ctx); // 5

      Pepe_OpenElement(ctx); // 6

        Pepe_OpenElement(ctx); // 7
        Pepe_CloseElement(ctx); // 7
        
        Pepe_OpenElement(ctx); // 8
          element = Pepe_GetOpenedElement(ctx);
          element->options.frameType = PEPE_FRAME_TYPE_GROW;
          Pepe_OpenElement(ctx); // 9
          Pepe_CloseElement(ctx); // 9
        Pepe_CloseElement(ctx); // 8
      Pepe_CloseElement(ctx); // 6
    Pepe_CloseElement(ctx); // 2

  Pepe_CloseElement(ctx); // 1
